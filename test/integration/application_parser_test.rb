  ...
  @fixtures.each do |app|
    ...
    test "sets people and applicants properly #{app[:name]}" do
      ...
      @people.each do |person|
        ...
        (json_applicant['Income']['Qualified Winnings'] || []).each_with_index do |input, index|
          assert_equal person.income[:qualified_winnings][index], {amount: input["Amount"].to_i, date: Date.parse(input["Date"])}
        end
      end
